...
int logicalShift(int x, int n) {
    return ((((!n)<<31)>>31)&x)+((((!!n)<<31)>>31)&((~((1<<31)>>(n+(~0))))&(x>>n)));
}
/* 
...
int remainderPower2(int x, int n) {
    int xsign=x>>31;
    int absval=(xsign&(~x+1))+((~xsign)&x);
    int quotient=(absval>>n)<<n;
    quotient=(xsign&(~quotient+1))+((~xsign)&quotient);
    return (x+(~quotient)+1);
}
/* 
...
unsigned floatAbsVal(unsigned uf) {
    unsigned nosignuf=uf<<1;
    unsigned exp=(nosignuf>>24);
    if(exp==0xFF){
        if(uf<<9)return uf;
    }
    return nosignuf>>1;
...
unsigned floatScale4(unsigned uf) {
    ...
    unsigned exp=(((uf<<1)>>24)+2);
    unsigned frac=((uf<<9)>>9);
    if(!((uf&0x7F800000)^0x7F800000))return uf;
    if(exp==2){
        frac<<=2;
        if(frac&0x01800000){
            if(frac&0x01000000)exp=2;
            else exp=1;
            frac>>=1;
            frac&=0x007FFFFF;
        }
...
unsigned floatNegate(unsigned uf) {
    if((!((uf&0x7F800000)^0x7F800000))&&(uf<<9)){
        return uf;
    }
    return ((~uf)&0x80000000)|((uf<<1)>>1);
}
/*
...
int satMul2(int x) {
    ...
    int xsign=x>>31;
    int msign=mul2>>31;
    return (xsign&(xsign^msign)&tmin)+((~xsign)&(xsign^msign)&tmax)+(~((xsign^msign))&mul2);
}
/* 
...
int signMag2TwosComp(int x) {
    int tmin=1<<31;
    int xsign=x>>31;
    return ((xsign)&(((!!(tmin^x))<<31)>>31)&(tmin|(~x+1)))+((~xsign)&x);
}
